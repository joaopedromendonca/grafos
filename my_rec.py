...
GRAU_DE_CORTE_SIMILARIDADE = 0.7

GRAU_COMPATIBILIDADE = 0.8
...
class Grafo_de_similaridade:
    ...
    # salva os filmes avaliados por usuarios no formato:
    # filmes_por_usuarios[usuario][(usuario,filme)] = nota
    filmes_por_usuarios = {}
    ...
    # salva no formato:
    # usuario_gosta[usuario][genero] = (media, quantidade, total, tba ou tma)
    # media -> media das notas em que o usuario avaliou bem/mal o genero (respectivamente usuario_gosta/usuario_n_gosta)
    # quantidade -> quantidade de avaliações boas/ruins do genero (respectivamente usuario_gosta/usuario_n_gosta)
    # total -> total de filmes avaliados pelo usuario
    # tba -> total de boas avaliações do usuario
    # tma -> total de mas avaliações do usuario
    # os 2 valores totais acima são influenciados pelo valor das notas de corte: NOTA_BOA e NOTA_RUIM
    usuario_gosta = {}
    usuario_n_gosta = {}
...
def top_similares(usuario, g, n_users):
    # lista das notas do top 10 usuarios compativeis
    top_n = []
...
def generos_por_usuario(usuario, g):
    ...
    for filme in g.filmes_por_usuarios[usuario]:
        ...
        elif g.filmes_por_usuarios[usuario][filme] < NOTA_RUIM:
            total_mas_av += 1
            for genero in g.generos_dos_filmes[filme[1]]:
                if genero in g.usuario_n_gosta[usuario]:
                    g.usuario_n_gosta[usuario][genero].append(g.filmes_por_usuarios[usuario][filme])
                else:
                    g.usuario_n_gosta[usuario][genero] = [g.filmes_por_usuarios[usuario][filme]]
    ...
    for genero in g.usuario_n_gosta[usuario]:
        qt = len(g.usuario_n_gosta[usuario][genero])
        soma = sum(g.usuario_n_gosta[usuario][genero])
        media = soma / qt
        g.usuario_n_gosta[usuario][genero] = (media, qt, total_filmes, total_mas_av)
